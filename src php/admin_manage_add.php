...
<body class='container-fluid'>
  <main class=" row">
    ...
    <div class="col-md-9 col-lg-10 col-sm">
      <div class="">
        <h1 class="mb-3">Thêm mới người dùng</h1>

        <form name="frmCreate" method="post" action="admin_manage_add.php" class="form">
          <div class="form-group form-floating mb-3">
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="radio" name="permission" id="user" value="User">
          ...
          <div class="form-group form-floating text-black mb-3">
            <input type="text" name="nhapavatar" class="form-control" id="nhapavatar" placeholder="">
        ...
        if (isset($_POST["btn_add"])) {
          ...
          if (!empty($fullname) && !empty($username) && !empty($email) && !empty($phone) && !empty($birthday) && !empty($gender) && !empty($password) && !empty($re_password) && !empty($permission)) {
            ...
            if (mysqli_num_rows($result1) > 0) {
              $flag = false;
              echo "<script>alert ('Tên tài khoản này đã có người dùng. Vui lòng chọn tên đăng nhập khác!')</script>";
              exit();
            } else {
              $flag = true;
            }
            ...
            //Insert data
            if ($flag) {
              $sql = "INSERT INTO `user` (`TenNguoiDung`, `Username`, `email`, `phone`, `birthday`, `gender`, `Password`, `level`, `Avatar`) VALUE('$fullname', '$username', '$email', '$phone', '$birthday', '$gender','$password', '$permission', '$avatar')";
              $result3 = mysqli_query($conn, $sql);
              if ($result3) {
                echo "<script>
                                      const result = confirm('Thêm mới thành công. Quay về trang quản lý tài khoản người dùng?');
                                      if (result){
                                          window.location.href='admin_manage.php';
                                      }
                                      else{
                                          window.location.href='admin_manage_add.php';
                                      }
                                      </script>";
              } else {
